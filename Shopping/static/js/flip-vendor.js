...
(function(win) {
    ...
    fStruct.prototype = {
        ...
        init: function() {
            ...
            this.f.style.backgroundImage="";
            this.z.style.backgroundSize="cover";
    ...
    rotateEx.prototype = {
        ...
        initPositionArr: function() {
            var THIS = this;
            var apW = this.allParent.offsetWidth,
                apH = this.allParent.offsetHeight;
            THIS.positionArr = [];
            $.each(this.parentEles, function(i, item) {
                var x = 0,
                    y = 0,
        ...
        //状态切换相关  代码增加图片更换
        faceIndex: function(index,img) {
            if (this.structArr[index].nowShow == "f") {
                this.structArr[index].change();
                this.structArr[index].z.style.backgroundImage="url("+img+")";
            }
        },
    ...
    $.fn.rotateEx = function(opt) {
        ...
        rotateObj.allTypeAmt = function() {
            ...
            if (this.type === "allBack" && window.isFlip) {
                window.isFlip = false;
        ...
        allEle.on(defOpt.clickName, function(e) {
            ...
            var self = this;
            // 该牌展示中奖图片，并翻牌
            var fnFlip = function(img) {
                var index = allEle.index(self);
                var bool = true;
                if (bool && isClick === false && testF()) {
                    isClick = true;
                    clickEle = self;
                    rotateObj.faceIndex(index,img);
                }
            };
            defOpt.getPrize(fnFlip);
        });

        // 随机打乱奖项
        o.reset = function() {
            var allEle = this.rotate.faceEles; // 正面
            var bufChild = null;
        ...
        o.toallFace = function() {
            // 如果全部是正面，全部翻身展示背面
            o.rotate.allFace();
        }

        o.alldiffer = function() {
            o.rotate.alldiffer();
        }

        o.backIndex = function(index) {
            o.rotate.backIndex();
        }
